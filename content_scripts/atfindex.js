...
var savePageProfile = 1;       //0:Nothing, 1:Save statistics, 2:Stats + Page profile, 3:Stats + Page profile + Timing, 4:Full log
...
function calculateATF(){
    ...
    //It seems firefox does not support first paint
    ...
    sendBlobToBackground(obj, filename)
    console.log(y)
}
...
//     console.log("Saving object to file: " + filename, "DEBUG");
...
function sendBlobToBackground(object, filename){
  console.log("Saving object to file: " + filename, "DEBUG");
  var blob = new Blob([JSON.stringify(object)], {type: "text/plain;charset=utf-8"});
  var sending = browser.runtime.sendMessage({
    msg: "aatf",
...
//RUM SPEED INDEX from this not working

//RUMSpeedIndex from WPO-Foundation
	  ...
	  for (var i = 0; i < elements.length; i++) {
		...
		if (el.tagName == 'IFRAME') {
		  try {
			var rect = GetElementViewportRect(el);
			if (rect) {
			  var tm = RUMSpeedIndex(el.contentWindow)[0];
			  if (tm) {
				rects.push({'tm': tm,
							'area': rect.area,
							'rect': rect});         
...
console.log("atfindex.js loaded");